...
void MyScene::keyPressEvent(QKeyEvent *event){
    ...
    case Qt::Key_V:
        m_jumpAnimation->setDuration(4000);
        m_jumpHeight = 500;
        if(bigMario){
    ...
    if(bigMario){
        ...
        if (direction > 0){
            ...
            if(diff > 800){
                ...
                g_life_count->setPos(dx + g_life_count->pos().x(), g_life_count->y());
                m_countLife->setPos(dx + m_countLife->pos().x(), m_countLife->y());
...
void MyScene::initPlayField(){
    ...
    m_countLife = new BackgroundItem(QPixmap(":images/count-life"));
    m_countLife->setPos(165, m_groundLevel -652 );
    addItem(m_countLife);
    ...
    g_life_count = new CoinCounter();
    g_life_count->setPos(200, m_groundLevel - g_life_count->boundingRect().height()-602);
    addItem(g_life_count);
    ...
    addItem(m_bomb2);
    connect(this->m_bomb2, SIGNAL(marioSizeStatus(int)),this, SLOT(setMarioSize(int)));
    ...
    addItem(m_redTurtle);
    connect(this->m_redTurtle, SIGNAL(marioSizeStatus(int)),this, SLOT(setMarioSize(int)));
    ...
    addItem(m_spiny);
    connect(this->m_spiny, SIGNAL(marioSizeStatus(int)),this, SLOT(setMarioSize(int)));
...
/////////////////////ycr code///////////////////////////////
/// \brief MyScene::checkCollidingGiantGoomba

//void MyScene::checkCollidingGiantGoomba() {
//    if(bigMario){
//        QList<QGraphicsItem*> items = collidingItems(m_player);
//        foreach (QGraphicsItem* item, items){
//            Goomba* g = qgraphicsitem_cast<Goomba*>(item);
//            if(g){
//                qDebug() << "goomba";
//                removeItem(g);
//            }
